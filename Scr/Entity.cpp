...
namespace Entity {
	...
	* @param ubSizePerEntity	エンティティごとの Uniform Buffer のバイト数
	* @param bindingPoint		エンティティ用 UBO のバインディングポイント
	* @param ubName				エンティティ用 Uniform Buffer の名前
	*
	...
	*			これ以上エンティティを追加できない場合はnullptrを返す
	...
	*/
		...
		const TexturePtr& texture, const Shader::ProgramPtr& program, Entity::UpdateFuncType func)
	{
		if (freeList.prev == freeList.next) {
			std::cerr << "WARNING in Entitiy::Buffer::AddEntity: 空きエンティティがありません" << std::endl;
			return nullptr;
		}
