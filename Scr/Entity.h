...
namespace Entity {
	...
	class Buffer;
	using BufferPtr = std::shared_ptr<Buffer>;		///<エンティティのバッファポインタ型.
	...
	public:
		///状態更新関数型.
		using UpdateFuncType =
			std::function<void(Entity&,void*,double,const glm::mat4&,const glm::mat4&)>;
	...
	*/
	...
	private:
		...
		UniformBufferPtr ubo;				///<エンティティ用の　UBO
