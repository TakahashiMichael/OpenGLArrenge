...
{
public:
	typedef std::function<void(double)> UpdateFuncType;///<更新関数のポインタ
	///カメラデータ.
	struct CameraData {
		glm::vec3 position;
		glm::vec3 target;
	...
	//入力を取る関数
	bool GetComand(GLFWEW::Window::COMMANDLIST)const;
...
private:
	...
	UniformBufferPtr uboVertex;
	UniformBufferPtr uboVertexScreen;
	UniformBufferPtr uboLightScreen;

	Shader::ProgramPtr progTutorial;
	Shader::ProgramPtr progColorFilter;
	Shader::ProgramPtr progScreen;
